...
public class ProductosDAO {
    ...
    public boolean insertarProductos(Productos producto) {
        String sql = "INSERT INTO productos (nombre, descripcion, proveedor, precio, fecha_registro, estado) VALUES (?, ?, ?, ?, ?,?)";
        try (PreparedStatement ps = con.prepareStatement(sql)) {
            ps.setString(1, producto.getNombre());
            ps.setString(2, producto.getDescripcion());
            ps.setString(3, producto.getProveedor());
            ps.setDouble(4, producto.getPrecio());
            ps.setDate(5, producto.getFecha_registro());
    ...
    public boolean actualizaProducto(Productos producto) {
        String sql = "UPDATE productos SET nombre = ?, descripcion = ?, proveedor = ?, precio = ?, fecha_registro = ?, estado = ? WHERE id_producto = ?";
        try (PreparedStatement ps = con.prepareStatement(sql)) {
            ps.setString(1, producto.getNombre());
            ps.setString(2, producto.getDescripcion());
            ps.setString(3, producto.getProveedor());
